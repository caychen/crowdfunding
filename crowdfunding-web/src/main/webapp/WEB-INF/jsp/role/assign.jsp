...
<script type="text/javascript">
	$(function () {
		...
		var setting = {
			...
			async: {
				enable: true,
				url: '${ctx}/role/loadpermission?roleId=' + ${param.id},
			},
			view: {
				selectedMulti: false,
	...
	function doAssign(){
		...
		if(nodes.length == 0){
			layer.msg("请选择需要分配的许可信息！", {icon: 5, shift: 6, time: 2000}, function(){});
		}else{
			var nodeArray = [];
			...
			$.when($.ajax({
				...
				data: {
					roleId: ${param.id},
					permissionIds: nodeArray.join(",")
				}
			})).then(function(data){
				layer.msg("许可分配操作成功", {icon: 6, time: 2000}, function(){});
			}, function(error){
				layer.msg("许可分配操作失败", {icon: 5, shift: 6, time: 2000}, function(){});
			});
		}
	}
